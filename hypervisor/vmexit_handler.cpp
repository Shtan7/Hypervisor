...
namespace hh::hv_event_handlers
{
  ...
  void vmexit_handler::vm_resume() noexcept
  {
    ...
    uint64_t error_code = {};
    __vmx_vmread(static_cast<size_t>(vmx::vmcs_fields::vm_instruction_error), &error_code);
    __vmx_off();
  ...
  {
    common::cpuid_t cpu_info = {};

    __cpuidex(reinterpret_cast<int*>(&cpu_info), static_cast<int32_t>(regs->rax), static_cast<int32_t>(regs->rcx));

    regs->rax = cpu_info.eax;
    regs->rbx = cpu_info.ebx;
    regs->rcx = cpu_info.ecx;
    regs->rdx = cpu_info.edx;
  }
  ...
  void kernel_hook_assistant::handle_ept_violation(common::guest_regs* regs, vcpu* cpu_obj)
  {
    ...
    ept::hooked_page_detail* hooked_page;
