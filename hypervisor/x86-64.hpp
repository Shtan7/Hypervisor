...
namespace hh::x86
{
  ...
  namespace msr
  {
    template<class T> inline auto read(uint64_t offset) noexcept
    {
      if constexpr (std::is_integral_v<typename T::result_type>)
      {
        return __readmsr(T::msr_id + offset);
      }
      else
      {
        return typename T::result_type{ .all = __readmsr(T::msr_id + offset) };
      }
    }

    template<class T> inline auto read() noexcept
    {
      if constexpr (std::is_integral_v<typename T::result_type>)
      {
        return __readmsr(T::msr_id);
      }
      else
      {
        return typename T::result_type{ .all = __readmsr(T::msr_id) };
      }
    }
