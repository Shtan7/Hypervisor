    ...
    struct gdt_entry_t
    {
      ...
      const gdt_entry_t& at(segment_selector_t selector) const noexcept
      {
        return *reinterpret_cast<gdt_entry_t*>(
          uint64_t(base_address()) + selector.flags.index * 8
          );
      }
...
#pragma pack(pop)
    ...
    namespace msr
    {
      template<class T> auto read(uint64_t offset) noexcept;
      template<class T> auto read() noexcept;
      template<class T> void write(uint64_t value) noexcept;
      template<class T> void write(T reg) noexcept;
    }
    ...
    struct segment_t
    {
      ...
      segment_t(descriptor_table64_t descriptor_table, T segment_selector) noexcept
      {
        ...
        else
        {
          ...
          access.all = static_cast<uint16_t>(__read_ar(selector.all) >> 8);
          access.unusable = !selector.all;
          access.limit_high = 0;
          limit = __read_sl(selector.all);
          ...
          {
            if (selector.flags.table == segment_selector_t::table_ldt) __int2c();

            const auto& table_entry = selector.flags.table
